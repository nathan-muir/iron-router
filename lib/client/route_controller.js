...
RouteController = Utils.extend(RouteController, {
  ...
  lookupWaitOn: function () {
    return this._collectHooks('waitOn');
  },
  ...
  _run: function () {
    ...
    self._computation = Deps.autorun(withNoStopsAllowed(function () {
      ...
      // Sometimes you want to do local permission checks before you load data
      Deps.autorun(withNoStopsAllowed(function () {
          self.runHooks('onEachRun');
      }));
      ...
      Deps.autorun(function (c) {
        ...
        // these hooks are called before the template has changed.
        self.runHooks('onBeforeSwitch', [], function (paused) {
          if (self.isStopped || paused)
            return;
          ...
          self.runHooks('onBeforeAction', [], function (paused) {
            ...
            // don't modify the `data` until we know the action will be called.
            Deps.autorun(function (c) {
              if (self.ready()) {
